...
object MatrixInjector {
    ...
    private fun checkInjection(file: File): Boolean {
        return (
//                file.absolutePath.contains("build" + File.separator + "intermediates" + File.separator)
...
//                    && isProjectModuleJar(file.absolutePath, modules)
    ...
    //复制方法
    fun realInjectClass(src: File, des: String) {
        //把文件里面内容放进数组
    ...
    private fun realInject(file: File, out: String) {
        try {
            ...
            if (file.path.endsWith(".class")) {
                val injectData = needInject(file.getAbsolutePath())
                if (injectData != null) {
    ...
    private fun needInject(path: String): InjectData? {
        if (Main.builderExtension == null || CollectUtil.isEmpty(Main.builderExtension?.getNameSet())) {
            return null
        }
        for (name in Main.builderExtension!!.getNameSet()) {
            if (!StringUtil.isEmpty(name)) {
//                Log.i("path=" + path + ",name=" + name)
